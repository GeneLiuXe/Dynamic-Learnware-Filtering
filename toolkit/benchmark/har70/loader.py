...
logger = get_module_logger("Har70Benchmark")
...
class DataLoader:
    ...
    def regenerate_data(self):
        new_df_list = []
        label_class = [1, 3, 6, 7, 8]
        mapping = {1: 0, 3: 1, 6: 2, 7: 3, 8: 4}
        raw_path = os.path.join(RAW_DATA_DIR, "har70plus")
        for filename in os.listdir(raw_path):
            file_path = os.path.join(raw_path, filename)
            df = pd.read_csv(file_path)
            label_list = [item[0] for item in df["label"].value_counts().items()]
            if set(label_class).issubset(set(label_list)):
                df = df.drop(columns=["timestamp"])
                df = df[df["label"].isin(label_class)]
                df["label"] = df["label"].map(mapping)
                new_df_list.append(df)

        for idx, df in enumerate(new_df_list):
            df_X = df.drop(columns=["label"])
            df_y = df["label"].to_numpy().reshape(-1, 1)
