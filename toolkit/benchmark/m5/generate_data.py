...
def add_lags_rollings(grid_df):
    ...
    grid_df = grid_df.assign(**{"{}_lag_{}".format(col, l): grid_df.groupby(["id"])[col].transform(lambda x: x.shift(l)) for l in LAG_DAYS for col in [TARGET]})
    ...
    for d_shift in [1, 7, 14]:
        for d_window in [7, 14, 30, 60]:
            ...
            grid_df[col_name] = grid_df.groupby(["id"])[TARGET].transform(lambda x: x.shift(SHIFT_DAY + d_shift).rolling(d_window).mean()).astype(np.float16)
...
def split_data(df, store, dept, fill_flag=False):
    ...
    print(store, dept, train_df.shape, val_df.shape)

    suffix = f"_fill" if fill_flag else ""
    train_df.to_pickle(os.path.join(PROCESSED_DATA_DIR, f"train_{store}_{dept}{suffix}.pkl"))
    val_df.to_pickle(os.path.join(PROCESSED_DATA_DIR, f"val_{store}_{dept}{suffix}.pkl"))
...
def split_m5():
    ...
    for store in store_list:
        grid_df_store = grid_df[grid_df["store_id"] == store]
        for dept in dept_list:
            # split_data(grid_df_store[grid_df_store["dept_id"] == dept], store, dept)
            split_data(grid_df_store[grid_df_store["dept_id"] == dept], store, dept, True)
